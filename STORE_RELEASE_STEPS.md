...
- `kind` — `ota` или `build`;
- `revision` — для OTA это **update id**, который печатает `eas update` (не
  branch и не group), для магазинного билда — `nativeBuildVersion` (то, что
  видно в `getFullVersion()` в скобках);
...
1. **OTA скачан** — «Обновление готово», тап применяет его сразу
   ...
   уведомление появляется только если запись о выпуске уже есть.
...
Запись можно завести и после публикации: пока её нет, бэкенд отвечает 204, а
приложение переспрашивает на каждом старте **в течение недели** с первого
вопроса — уведомление придёт и тем, кто успел обновиться раньше, чем текст
написали. После недели молчание считается окончательным (иначе выпуск без
записи — а это большинство багфиксов — спрашивал бы вечно). На свежей
установке «что нового» не показывается: сравнивать не с чем.
