...
- `npm run e2e:ids` — список всех `testID` из `screens/`/`components/`. `npm run e2e:lint` — сверяет `id:`-селекторы всех флоу с этим списком и проверяет синтаксис (`maestro check-syntax`). Устройство не нужно, работает за секунду.
- `npm run e2e:scratch -- <файл>` — прогон одноразового флоу из `.maestro/scratch/` (в `.gitignore`, в батч не попадает).
- `npm start` — Expo dev server. `npm run ios` / `npm run android` — нативная сборка.
- `npm run i18n:extract` — извлечение строк локализации.

## Работа с e2e (Maestro)

- **Перед любой правкой или разбором флоу прочитай `RELEASE_CHECKLIST.md` §1** — там состав флоу, поведение эмулятора (авиарежим, wifi, залипание HTTP-стека), практика прогонов и инструменты отладки. Известные блокеры и тупики уже описаны там; §3 — что именно проверяет каждый офлайн-флоу, §6 — очередь следующих. И загляни в `git status`: незакоммиченный `.md` в корне — почти всегда рабочая записка незавершённого захода, читать её до начала работы.
- **Селекторы не выдумывай — снимай с устройства.** `.maestro/ui.sh` печатает иерархию живого экрана, отфильтрованную до `testID` и текстов; `npm run e2e:ids` — что вообще есть в коде. Скриншот показывает, что нарисовано, но не то, каким селектором до этого дотянуться, — а цена ошибки в селекторе это полный прогон (~90 с).
- **Итерируй на скретч-флоу, а не на боевом.** Флоу без `launchApp` стартует из текущего состояния приложения, и проверка одного шага стоит секунды вместо полутора бутстрапов.
- **Разбор падения начинай с логов приложения.** При падении Android-прогона `run.sh` кладёт `.maestro/.logs/<flow>.logcat` — там строки самого приложения (`[AUTH API ERROR]` и прочее), то есть причина, а не симптом. Дальше — артефакты Maestro в `~/.maestro/tests/<timestamp>/`.
- **Wifi на эмуляторе не включать.** Явно включённый wifi переживает авиарежим, и офлайн-флоу молча начинают тестировать пустоту — зелёные там, где должны быть красные. Изоляция сети делается только авиарежимом поверх мобильных данных.
