...
`online-diary-add-another` (09.09.2026) покрывает кнопку «Сохранить и
добавить ещё» в дневнике — цепочку из двух наблюдений подряд. Проверяет
ровно то, чего юнит-тесты увидеть не могут: после сохранения список видов
открывается **сам**, на сброшенной форме. Цепочка внутри экрана
(`openSpeciesSignal` → `ObservationForm` → `DropdownInput`) юнитами покрыта
до последнего шага, но «модалка действительно всплыла» — вопрос устройства,
тем более что экран на время мутации размонтирует форму в `LoadingOverlay`
и дропдаун открывается **на монтировании**, а не по смене пропа. Второй вид
берётся строкой `option-row-1`: только что сохранённый вид дневник уже
содержит, и его строка в списке выключена (`existingSpecies`). Кнопка
получила `testID` (`observation-save-add-another-button`) — до этого до неё
можно было дотянуться только по переведённому тексту. **Написан, но на
устройстве ещё не прогонялся** — теги стоят на обе платформы, первый прогон
делать поодиночке.
...
места. **Прогнан только на iOS** (зелёный); теги стоят на обе платформы, но
на Android первый прогон стоит делать поодиночке. Три вещи, которые он
выяснил на устройстве и которые стоит помнить, берясь за соседние флоу:

- **Список мест отфильтрован глобальным фильтром страны**, и флоу, ставящий
  `setLocation` в Париж, создаёт место, которого в этом списке потом нет
  вовсе (аккаунт — со страной по профилю). Первая версия чистила за собой
  через список и не находила там собственное место. Сейчас чистка идёт через
  кнопку «открыть место» в самом пикере (`place-dropdown-open`, новый
  `testID`), а не через список.
- **Имя места уникально в пределах профиля**
  (`place_unique_profile_name_no_external` в `myapi/models.py`), поэтому
  фиксированное имя ломает каждый следующий прогон после упавшего: сохранение
  отклоняется, а флоу падает шагом позже, вдалеке от причины. Имя собирается
  из `Date.now()` в `evalScript`.
- **Территорию формы задаёт профиль аккаунта**, а он разный на платформах
  (Франция у одного, Беларусь у другого), тогда как подсказка сравнивает
  только места внутри территории формы. Поэтому якорное место создаётся не из
  списка мест, а из самой формы — `onPlaceCreated` переводит форму на
  территорию нового места, и обе стороны сравнения приходят из одного
  мок-фикса.

Четыре `account-*` написаны 31.08.2026 вместе с самим разделом «аккаунт»
(см. [ACCOUNT_PLAN.md](ACCOUNT_PLAN.md)) и **прогнаны на Android 01.09.2026** —
все четыре зелёные, в том числе в составе полного батча. **На iOS они
по-прежнему не прогонялись**, так что там первый прогон по-прежнему стоит
делать поодиночке и снимать селекторы `.maestro/ui.sh`, а не считать их
проверенными.

Опасение оправдалось: `npm run e2e:lint -- --syntax` проходил, а первый же
прогон на устройстве нашёл три вещи — одну в среде и две в приложении:

- диалог Google Password Manager поверх поля ввода — среда, а не код;
  разобран в «Эмулятор и сеть» ниже, закрывается
  `common/dismiss-autofill.yaml`;
- **баг приложения**: глобальный `BottomSheet` умирал после связки «меню →
  подтверждение поверх меню → Подтвердить» — примерно раз в три случая
  `present()` монтировал шит и не открывал его, после чего в приложении
  переставали работать все меню и подтверждения до перезапуска процесса.
  Причина — гонка `BottomSheetModal` с `@gorhom/portal` при разрушении шита:
  модалка снимает свою portal-запись, ре-рендер её возвращает, а
  размонтирующийся `Portal` уже не убирает (`handlePortalOnUnmount` выходит
  рано на статусе `INITIAL`), и следующий `present()` реконсилится в мёртвый
  узел. Исправлено в `components/ui/UniversalBottomSheet.tsx`: `present()`
  проверяет результат и, если шит за 600 мс не сообщил, что открылся, зовёт
  `snapToIndex(0)` (тесты — в `__tests__/UniversalBottomSheet.test.tsx`);
- **баг приложения**: строки меню в `EmailsScreen` не закрывали шит, и меню
  висело поверх собственного результата. `overflowMenu` делает это за
  header-меню, а экран звал `BottomSheet.showMenu` напрямую и про правило
  забыл; исправлено, кроме строки «Remove» — она открывает подтверждение
  поверх (`opensAnotherSheet`).

Почему не всё общее:

- `offline-*` — `toggleAirplaneMode` есть только на Android (раздел 6);
- `account-emails` — действия над строкой адреса живут в
  `BottomSheet.showMenu`, а он на iOS до Maestro не доходит (см. ниже).
  Пропустить эти шаги на iOS нельзя: без меню флоу не сможет удалить
  добавленный им адрес и будет засорять общий аккаунт каждым прогоном.
  Стоит перепроверить — то замерение делалось на меню **без** `title`, а это
  передаёт адрес заголовком, то есть ровно та разница, которой чек-лист
  объясняет доступность шторки фильтров;
- `list-sort-persist` / `list-filters` — iOS схлопывает содержимое
  ...
  заголовке (`title` у `BottomSheet.showContent`). Меню
  (`BottomSheet.showMenu`) заголовка не передаёт и на iOS недостижимо
  целиком — ни по `testID`, ни по тексту пунктов (замерено скретч-флоу
  2026-08-26 при написании `species-from-my-lists`; на Android те же пункты
  видны по label). Проверять на iOS можно только то, что видно снаружи
  шторки, — что и делает `species-from-my-lists`. То же самое, что не
...
`species-from-my-lists.yaml` (2026-08-26) — прогнан на симуляторе и эмуляторе,
iOS и Android зелёные. Написан вместе с правкой переходов на карточку вида: до
него ни один флоу не доходил до экрана вида из личной половины приложения
(только из каталога и по диплинку), а именно она и менялась.

С первого раза он был красным, и ровно на том, чего `npm run e2e:lint` не
ловит: проверял `bottom-sheet-cancel-button` после тапа по строке чеклиста, а
этот `testID` живёт только в confirm-режиме шторки — у меню кнопки отмены нет
вовсе, закрывается оно свайпом вниз (`enablePanDownToClose`). `e2e:lint`
пропустил это потому, что id существует в коде; он не знает, в каком режиме тот
рендерится. Ответ, чем заменить, снят скретч-флоу, а не угадан (см. пункт про
`showMenu` в списке платформенных различий выше).

Полный iOS-батч после этой правки уронил три чужих флоу, и оба падения стоит
запомнить — ловятся они только прогоном, `e2e:lint` про такое ничего не знает.

- `online-create-place` / `online-create-place-location-denied` тапали
  `id: "section-Places"` без скролла. `Sections` — последний блок дашборда, а
  его высота зависит от данных аккаунта («новые виды», карточка алертов,
  «редкие рядом»), так что секция держалась у самого края экрана и флоу жили на
  этом везении. Карточка выше выросла на одну строку кнопки — и секция вышла за
  вьюпорт. На iOS это выглядит не как «элемент не виден», а как **Element not
  found**: в иерархию XCUITest попадает только видимая часть скролла (проверено
  `.maestro/ui.sh` — дерево обрывается ровно на последнем видимом элементе).
  Лечится `scrollUntilVisible` перед тапом; он no-op, когда элемент и так в
  кадре.
- `online-nested-observation` тапал строку наблюдения по названию вида
  (`.*${output.nestedSpecies}.*`) и уезжал на карточку вида. Причина — не
  вёрстка, а `accessibilityLabel`: миниатюре вида дали в метку название птицы,
  и в строке стало два элемента с одинаковым текстом, ведущие в разные места.
  Тем же самым это ломало и VoiceOver. Метка миниатюры теперь называет
  действие (`species_details`), а не птицу; см. комментарий в
  `components/Taxonomy/SpeciesThumb.tsx`. Правило на будущее: у вложенного
  контрола метка обязана отличаться от текста строки, иначе текстовый селектор
  выберет не то — и молча.

Блок «птицы дня» в нём безусловный. Сначала он был обёрнут в `when: visible` —
на случай страны без птицы дня, — но такой страны не бывает: бэкенд собирает
пул кандидатов из объединения статистики сообщества и чек-листа страны
(коммит `b0c03066`, `birdofday_selector.get_bird_of_day_candidates`), так что
птица дня есть у любой страны с чек-листом, а страна есть у каждого e2e-аккаунта
— на это и так опирается первая половина флоу. Условие давало только одно:
возможность молча пропустить проверку.
...
Четыре вещи, каждая из которых уже съедала по заходу отладки. Первые три — про
сеть: прежде чем чинить офлайн-флоу, проверять именно их, иначе среда выглядит
как регрессия в приложении. Четвёртая — системный диалог, который вмешивается
в любой флоу, где что-то вводится в поле логина или пароля.
...
- **Диалог Google Password Manager перехватывает фокус на полях логина**
  (замерено 01.09.2026 на `account-emails` и `account-change-password`). На
  тап по полю с `autoComplete="email"` или по полю пароля Android 14+
  показывает не строку подсказок над клавиатурой, а диалог
  (`android:id/autofill_dialog_picker`, «Use your saved password for DiBird
  (Dev)?») — в эмуляторе он есть всегда, потому что батч сам логинится
  сохранёнными кредами. Диалог забирает фокус, поэтому `pasteText` уходит в
  него, а не в поле, и накрывает низ экрана: всё, что ниже сфокусированного
  поля, Maestro сообщает как «element not found», а не как «не видно» — по
  сообщению это неотличимо от опечатки в селекторе. Показывается не в каждом
  прогоне (после закрытия Android подавляет его на время), так что зелёный
  сегодня флоу краснеет завтра на ровном месте. Лечение — `runFlow:
  common/dismiss-autofill.yaml` сразу после тапа по такому полю: он ждёт
  анимацию и, если диалог на экране, закрывает его `Back`, оставляя поле
  сфокусированным с поднятой клавиатурой. Безусловный `Back` не годится: без
  диалога он просто уберёт клавиатуру.
...
- `DEADLINE_EXCEEDED` в Android-прогоне — залипший эмулятор, а не
  регрессия; каскадит на весь батч. Перезапуск эмулятора возвращает
  работоспособность, но это откат, а не лечение: у залипания есть
  воспроизводимая причина, см. следующий пункт.
- **`guest-deeplink-no-session` кладёт эмулятор на следующем флоу батча.**
  Воспроизведено трижды (27.08.2026), каждый раз одинаково: он —
  единственный Android-флоу с `launchApp: clearState: true`, то есть с
  `pm clear`, и по алфавиту стоит ровно перед `list-filters`. `pm clear`
  сносит и сессию, и AsyncStorage-маркер `species_images_prefetched`, и
  дисковый кэш картинок, поэтому `list-filters` логинится с нуля, а после
  логина `useSpeciesImagePrefetch` запускает прогрев **всех** миниатюр
  территории (Франция — 676 видов) в 6 потоков. Локальный бэкенд — это
  `manage.py runserver`, отдающий media сам, — начинает рвать соединения
  (`Load failed … EOFException` от Glide в logcat, 63 штуки за 7 секунд),
  Glide ретраит, сетевой стек AVD захлёбывается, и гость встаёт целиком:
  logcat замолкает на той же секунде, `adb shell echo` не отвечает,
  `adb devices` показывает `offline`, qemu держит 100–260% CPU. Наружу
  это выглядит как зависание на шаге `Tap on (Optional) "Not Now"` — он
  просто первый после логина, кто дёргает `viewHierarchy`, и досиживает
  свои 120 с до `DEADLINE_EXCEEDED`. Сам шаг ни при чём (это iOS-овский
  промпт iCloud Keychain, на Android его нет никогда).
  Прогоны тех же флоу поодиночке проблемы не дают: маркер прогрева цел,
  и шторма не возникает. Приложение при этом ведёт себя штатно — прогрев
  на то и написан; узкое место — связка «AVD + runserver», а не код.

  **Исправлено 27.08.2026 переносом media мимо Django.** В `docker/dibird_local`
  бэкенда появился сервис `media` (nginx, порт 8080), отдающий `/media/` прямо
  с диска, а `EXPO_PUBLIC_MEDIA_URL` в `.env.development.local` переехал на
  `http://192.168.0.103:8080`. Локальная схема стала такой же, как на проде,
  где media и так живёт на отдельном хосте (`s.dibird.com`), а `/media/` с
  основного домена отдаёт 410. Тот же файл: nginx — 5 мс, Django — 42 мс.
  После правки связка `guest-deeplink-no-session` → `list-filters` проходит
  зелёной с полным прогревом.

  **`.env.development.local` не коммитится** (`.gitignore`), так что на другой
  машине порт media придётся прописать руками — иначе всё вернётся к прогреву
  через Django и к этому же зависанию. Проверить раздачу можно так:
  `curl -o /dev/null -w '%{http_code}\n' http://localhost:8080/media/<любой файл>`.
  Промахи по гипотезам, чтобы не проверять их заново: увеличение RAM эмулятора
  с 2 до 6 ГБ не помогает, снижение `PREFETCH_CONCURRENCY` с 6 до 2 — тоже;
  дело в объёме запросов, а не в параллельности или памяти.
...
Управление аккаунтом (раздел появился 31.08.2026, разбор — в
[ACCOUNT_PLAN.md](ACCOUNT_PLAN.md)) — четыре флоу, все зелёные на Android с
01.09.2026 (на iOS ещё не прогонялись, см. раздел 1):

- `account-forgot-password.yaml` (обе платформы): выход из аккаунта → Login →
  «Forgot password?» → мусор вместо адреса не уходит на сервер → настоящий
  адрес → «письмо отправлено» → повторная отправка. Именно эта половина и
  падала 500 на бэке (`reverse('password_reset_confirm')` по несуществующему
  имени роута), и увидеть это можно только настоящим запросом. Пароль флоу не
  меняет **намеренно**: он лежит в `.maestro/.env.local`, и упавший на
  полпути флоу запер бы весь батч снаружи аккаунта. По ссылке из письма флоу
  не ходит — ключ живёт в mailpit, а не на устройстве.
- `account-change-password.yaml` (обе платформы): экран показывает поле
  «текущий пароль» (то есть `has_usable_password` доехал), несовпадение
  подтверждения не уходит на сервер, а неверный текущий пароль сервер
  отклоняет читаемым сообщением. Успешную смену не делает по той же причине,
  что и предыдущий; она покрыта `myapi/tests/test_password_api.py`.
- `account-emails.yaml` (Android): список с бейджами, у основного адреса
  действий нет, добавление → «не подтверждён» → повторное письмо → удаление.
  Подчищает за собой и за упавшим предыдущим прогоном.
- `account-linked-accounts.yaml` (обе платформы): экран грузит список с
  `GET /myapi/profile/social/` — эндпоинт годами существовал без единого
  вызывающего, этот экран первый. Кнопку привязки флоу не жмёт: дальше идёт
  системный диалог провайдера.

Ручными пунктами остаются: открытие ссылки сброса на устройстве (`xcrun
simctl openurl` / `adb shell am start` по адресу из mailpit), привязка и
отвязка провайдера, и подтверждение второго адреса по ссылке из письма
из-под авторизации.
...
- [x] Подсказка «рядом уже есть такое место» (`hooks/Place/useNearbyPlaces.ts`,
      `components/Place/NearbyPlaceSuggestion.tsx`) — в пикере формы и в
      редакторе места: `.maestro/online-nearby-place.yaml`, прогнан на iOS
      07.09.2026. Юнит-тесты покрывают пороги и оба тона карточки; на
      устройстве проверяется то, что им недоступно, — что живой GPS-фикс,
      реальный reverse geocode и только что созданное место действительно
      сходятся достаточно близко, чтобы карточка вообще появилась.
...
- Первый прогон `account-*` на устройстве (Android, 01.09.2026). Четыре флоу,
  написанные 31.08 вслепую, прогнаны поодиночке и полным батчем: 25 android-флоу
  за ~50 минут. Нашлись и **исправлены** два бага приложения — умирающий
  глобальный `BottomSheet` после «меню → подтверждение → Подтвердить» и
  не закрывающиеся строки меню в `EmailsScreen` (оба разобраны в разделе 1) —
  и одна ловушка среды, диалог Google Password Manager поверх полей ввода,
  вынесенная в `common/dismiss-autofill.yaml`. Попутно измерено про сам AVD:
  за длинную серию прогонов он залипает (`adb devices` отвечает `device`, а
  `adb shell` виснет намертво, qemu на ~100% CPU) — лечится только `kill -9` по
  qemu и холодным стартом; батч, запущенный с холодного старта
  (`-no-snapshot-load`), прошёл оба раза без единого залипания, тогда как
  прогоны поверх восстановленного снапшота умирали примерно раз в полчаса.
  Что осталось нестабильным и без залипаний: пара офлайн-флоу подряд иногда
  роняет следующий за собой — сеть AVD после авиарежима поднимается не сразу
  (см. «Эмулятор и сеть»), а первый запуск приложения после холодного старта
  доходит до главного экрана за ~45 с, дольше любого таймаута в
  `common/bootstrap.yaml`. Оба случая проходят с повтора: судить по
  одиночному прогону упавшего флоу, а не считать батч красным.
