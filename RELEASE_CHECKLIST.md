...
- CI (репозиторий сейчас на Bitbucket, одна ветка master — т.е.
  `bitbucket-pipelines.yml`, не GitHub Actions), гоняющий `npm run check` +
  `npm test` на каждый push в master (и, когда будет готова сборка для
  симулятора/эмулятора в CI, `npm run e2e` / `npm run e2e:android`) —
  сейчас всё из раздела 1 выполняется вручную разработчиком перед
  релизом. Низкие трудозатраты (один YAML, стандартный Node-образ), чисто
  дополнительная страховка от "забыл прогнать перед релизом", не блокирует
  ничего из этого документа — не сделано в этом батче просто по
  приоритету, не по сложности.
