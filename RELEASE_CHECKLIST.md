...
(см. [ACCOUNT_PLAN.md](ACCOUNT_PLAN.md)) и **на устройстве ещё не прогнаны** —
ни симулятора, ни эмулятора под рукой не было. `npm run e2e:lint -- --syntax`
их проходит, но он не ловит ровно тот класс ошибок, на котором спотыкался
`species-from-my-lists` (id существует в коде, но рендерится не в том режиме).
Первый прогон стоит делать поодиночке и с готовностью снимать селекторы
`.maestro/ui.sh`, а не считать их проверенными.
...
[ACCOUNT_PLAN.md](ACCOUNT_PLAN.md)) — четыре флоу, ни один ещё не прогнан на
устройстве:
