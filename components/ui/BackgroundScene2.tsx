...
const BackgroundScene2 = () => {
  ...
  // const skyColor = isDark ? "#0D1A0C" : "#1B2B1A";
  // const horizonColor = isDark ? "#1A2210" : "#2D3B1E";
...
`
  ...
  // Дальний ряд — чуть ниже и менее детальный:
  const treesFar = `
  0,${H}
  0,${H * 0.8}
  ${W * 0.08},${H * 0.7}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Ground */}
        <Polygon
          points={`0,${H * 0.85} 0,${H} ${W},${H} ${W},${H * 0.85}`}
          fill={treeColor}
          opacity={isDark ? 0.03 : 0.05}
        />

        <Polygon
          points={`0,${H * 0.82} 0,${H} ${W},${H} ${W},${H * 0.82}`}
          fill={treeColor}
          opacity={isDark ? 0.03 : 0.1}
        />

        {/* Birds in flight */}
        {/* дальняя левая пара — у левого края */}
        <Path
