...
- ~~Приложение **полностью за логином**~~ — снято 26-27.07 (1.1 и 1.2 ниже). Стена входа больше не первый экран; трение осталось **внутри**: разрешения на гео и пуши просятся сразу после логина, вне контекста ([App.tsx:148](App.tsx#L148), [hooks/usePushNotifications.ts:69](hooks/usePushNotifications.ts#L69)), нет онбординга, нет запроса оценки.
...
### 1.3 Онбординг (3 экрана + первый успех)
...
4. Онбординг (1.3) и ASO (1.4) — остаток фазы 1.
5. Импорт из eBird (2.1).
...
ручным пунктом [RELEASE_CHECKLIST.md](RELEASE_CHECKLIST.md) §2.
