...
### 1.1 Гостевой режим («Смотреть без регистрации»)

Бэк уже отдаёт каталог без аккаунта (`IsFromApp`), поэтому это работа только на
фронте:

- [constants/config.ts:35](constants/config.ts#L35) — расширить `publicEndpoints`
  (`/api/taxon/`, `/api/taxon-data/`, `/api/territory*`, `/api/search/`,
  `/api/news/`, `/api/page2/`), чтобы request-интерсептор
  [services/api.ts:168](services/api.ts#L168) не требовал токен.
- [navigation/AuthStack.tsx](navigation/AuthStack.tsx) — добавить в стек уже
  готовые экраны: `TaxonomyScreen`, `TaxonGroupDetailScreen`,
  `SpeciesDetailScreen`. Они не завязаны на профиль (проверить `useAuth`/
  `useProfile` в них перед подключением — при необходимости обернуть в
  условный рендер).
- [screens/WelcomeScreen.tsx](screens/WelcomeScreen.tsx) — третья кнопка
  «Посмотреть каталог птиц» под Apple/Google/Login.
- Мягкий upsell вместо жёсткой стены: у любого действия, требующего аккаунта
  («отметить как увиденное», «добавить в watchlist», «сохранить место»)
  — общий хук `useRequireAuth()` (новый, рядом с
  [hooks/useApiError.ts](hooks/useApiError.ts)), который показывает bottom-sheet
  «Создайте аккаунт, чтобы сохранить» через уже существующий
  [services/bottomSheet.ts](services/bottomSheet.ts).
...
1. **Шеринг-карточки.** Дёшево для соло: картинку генерит **бэк** (Django уже
   рендерит веб), приложение шерит ссылку `https://dibird.com/s/<token>` с
   OG-изображением; ссылка открывается в приложении через уже настроенные
   `applinks:dibird.com` + [linking.ts](linking.ts). Типы: лайфлист года,
   новый вид, достижение, результат квиза.
...
- **Фото в наблюдениях** — нужны модель, S3-хранилище, модерация и деньги за
  ...
  причина установки.
- **Территории/новости/глобальный поиск** (Приоритет 2-3 из
  [TAXONOMY_CATALOG_STATUS.md](TAXONOMY_CATALOG_STATUS.md)) — полезно, но
  на воронку не влияет; делать после фазы 2, кроме поиска, который нужен для
  гостевого режима.
...
1. Фаза 0 целиком (аналитика) — до всего остального.
2. Гостевой режим + онбординг + запрос оценки (1.1, 1.2, 5.1-5.2) — один релиз.
